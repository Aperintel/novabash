...
import { openCredential } from './crypto/index';
...
export async function validateWorkspace(
  ...
  // Validate every service group in parallel. Wall-clock equals the slowest
  // single vendor call rather than the sum, so an 8-service workspace fits
  // inside Vercel's 10-second hobby-tier function ceiling even if every
  // vendor takes 5 seconds.
  ...
  await Promise.all(
    groupResults.map(async ({ items, result }) => {
      await Promise.all(
        items.map((i) =>
          db
            .update(credentials)
